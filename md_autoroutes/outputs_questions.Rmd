...
# Typology: Center, Urban, Intermediate, Rural
sum(ch_aggregated_geolevels$swiss_data_muni.f[which(ch_aggregated_geolevels$nouvelle_typo == 1)])
sum(ch_aggregated_geolevels$swiss_data_muni.f[which(ch_aggregated_geolevels$nouvelle_typo == 2)])
sum(ch_aggregated_geolevels$swiss_data_muni.f[which(ch_aggregated_geolevels$nouvelle_typo == 3)])
sum(ch_aggregated_geolevels$swiss_data_muni.f[which(ch_aggregated_geolevels$nouvelle_typo == 4)])


# median
library(spatstat)
# region
weighted.median(ch_aggregated_geolevels$yes.X6730[which(ch_aggregated_geolevels$nouvelle_typo == 4)],
ch_aggregated_geolevels$swiss_data_muni.f[which(ch_aggregated_geolevels$nouvelle_typo == 4)], na.rm = TRUE)
# langue
weighted.median(ch_aggregated_geolevels$yes.X6730[which(ch_aggregated_geolevels$language == 1)],
ch_aggregated_geolevels$swiss_data_muni.f[which(ch_aggregated_geolevels$language == 1)], na.rm = TRUE)

dim(ch_aggregated_geolevels[which(ch_aggregated_geolevels$language == 1 ),])
dim(ch_aggregated_geolevels[which(ch_aggregated_geolevels$yes.X6730 > 0.5 & ch_aggregated_geolevels$language == 1),])

#Moyenne pondérée
sample_f = ch_aggregated_geolevels$swiss_data_muni.f[which(ch_aggregated_geolevels$road_project_n > 0 )]/sum(ch_aggregated_geolevels$swiss_data_muni.f[which(ch_aggregated_geolevels$road_project_n > 0 )])

sample_yes = ch_aggregated_geolevels$yes.X6730[which(ch_aggregated_geolevels$road_project_n > 0 )]
weighted.mean(sample_yes, sample_f)

#Moyenne pondérée typologies
sample_f = ch_aggregated_geolevels$swiss_data_muni.f[which(ch_aggregated_geolevels$nouvelle_typo == 4 )]/sum(ch_aggregated_geolevels$swiss_data_muni.f[which(ch_aggregated_geolevels$nouvelle_typo == 4 )])

sample_yes = ch_aggregated_geolevels$yes.X6730[which(ch_aggregated_geolevels$nouvelle_typo == 4 )]
weighted.mean(sample_yes, sample_f)

#Moyenne pondérée par pojet
condition = ch_aggregated$road_project_indiv == 5 # direct 1 bern 2 ge 3 st.g 4 schaff 5 basel
condition = ch_aggregated$road_project_indiv_n == 5
condition = ch_aggregated$road_project_indiv_n2 == 5
condition = ch_aggregated_geolevels$language == 4

sample_f = ch_aggregated_geolevels$swiss_data_muni.f[which(condition)]/sum(ch_aggregated_geolevels$swiss_data_muni.f[which(condition)])
sample_yes = ch_aggregated_geolevels$yes.X6730[which(condition)]
length(sample_yes)
weighted.mean(sample_yes, sample_f)
...
my_palette_langue = colorNumeric(
  palette = colorRampPalette(c("#66C2A5", "#FC8D62", "#8DA0CB",  "#E78AC3")),
  domain = NULL
)
ch_langue$color = c("#66C2A5", "#FC8D62", "#8DA0CB",  "#E78AC3")
...
t.test(residu_road[which(geoLevels$`Typologie urbain-rural` == 1)], mu = 0)

t.test(residu_road[which(as.character(ch_aggregated_geolevels$nouvelle_typo) == 4)], mu = 0)
length(which((as.character(ch_aggregated_geolevels$nouvelle_typo) == 3)))
```
...
```{r pressure, echo=FALSE}
ch_filtered = ch_aggregated_geolevels[ch_aggregated_geolevels$swiss_data_muni.f >= sort(ch_aggregated_geolevels$swiss_data_muni.f, decreasing = TRUE)[20], ]

palCal <- colorFactor(palette = "Set3", domain = unique(geoLevels$`Typologie des communes (9 types)`))
palCal <- colorFactor(palette = "Set3", domain = unique(ch_aggregated_geolevels$nouvelle_typo), reverse = TRUE)
...
leaflet(ch_aggregated_geolevels) %>%
  ...
  addPolygons(
    ...
    fillColor = ~palCal(ch_aggregated_geolevels$nouvelle_typo),
    fillOpacity = 0.9,
    color = "white",
    weight = 0.1,
    opacity = 1,
    ...
    label = ~paste0(ch_aggregated$NAME, ": ", ch_aggregated_geolevels$nouvelle_typo),
    smoothFactor = 0.2,
    # group = "MDS fac. 1",
  ) %>%
  addPolylines(
  ...
  addLabelOnlyMarkers(data=communes_centres_sorted[1:20,], ~longitude, ~latitude, label =  ~as.character(NAME), 
                      labelOptions = labelOptions(noHide = T, direction = 'top', textOnly = T))  %>%
  addScaleBar(position = "bottomright", options = scaleBarOptions(imperial = F)) %>%
  addLegend(
    colors = c("#d47c17", "#d44317"),
    position = "bottomright",
    labels = c("Motorway", "Voted projects"),
    opacity = 0.9
  ) %>%
  # addLegendLine(values = 0.2, color = "#d47c17",
  #                      orientation = 'horizontal', width = 30)  %>%
  # addLegendLine(values = "Voted projects", color = "#d44317",
  #                      orientation = 'horizontal', width = 30)  %>%
  # addLegendCustom(
  #   colors = c("#d47c17", "#d44317"),
  #   labels = c("Motorway", "Voted projects"),
  #   sizes = c(4, 4),  # épaisseur des lignes
  #   shapes = "line",
  #   position = "bottomright"
  # ) %>%
  # Ajout de la légende
  addLegend(
    ...
    colors = palCal(sort(unique(ch_aggregated_geolevels$nouvelle_typo))),
...
result <- ch_aggregated_geolevels %>%
  group_by(`Typologie.des.communes..9.types.`) %>%
  summarise(
    moy_ponderee_yesX6730 = sum(yes.X6730 * swiss_data_muni.f, na.rm = TRUE) / 
                             sum(swiss_data_muni.f[!is.na(yes.X6730)], na.rm = TRUE)
  )
